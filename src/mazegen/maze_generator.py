...
class MazeGenerator:
    """Generate and manage a maze representation.

    This class builds a maze using algorithms, add a logo
    pattern, finds paths, and can import/export maze data.

    Attributes:
        config (Config): Configuration used to initialize the maze.
        width (int): Maze width.
        height (int): Maze height.
        start (t_point): Entry coordinates in the maze (x, y).
        end (t_point): Exit coordinates in the maze (x, y).
        seed (int): seed value.
        output_file (str): Filepath where exported maze is saved.
        logo_file (str): Filepath of the logo.
        logo (list[list[int]]): list of list representing the logo.
    """
    def __init__(self, config: Config):
        """Initialize a MazeGenerator instance.

        Args:
            config (Config): Configuration object containing maze
                parameters.
